...
class awardedrcd extends page{
    ...
    public function main($op='display'){
	    ...
	    if($type=='1'){
	        ...
	        $totalrp=pdo_fetchcolumn("select sum(redpack) from ".tablename('hxq_newywym_token')." where uniacid={$uniacid} and isgrant=1 and actid={$act['id']}  and oauth_openid='{$_SESSION['oauth_openid']}' {$where} order by addtime desc");
	        $myawardeds=pdo_fetchall("select * from ".tablename('hxq_newywym_token')." where uniacid={$uniacid} and isgrant=1 and actid={$act['id']}  and oauth_openid='{$_SESSION['oauth_openid']}' {$where} order by addtime desc");
        }elseif($type=='3'){
            if ($_W['fans']['uid']){
	       ...
	       $myawardeds=pdo_fetchall("select credit1,isgrant,addtime from ".tablename('hxq_newywym_token')." where uniacid={$uniacid} and isgrant=1 and actid={$act['id']} and oauth_openid='{$_SESSION['oauth_openid']}' and type=3 order by addtime desc");
	       ...
	       load()->classs('coupon');
	       $myawardeds=pdo_fetchall("select * from ".tablename('hxq_newywym_token')." where uniacid={$uniacid} and isgrant=1 and actid={$act['id']} and oauth_openid='{$_SESSION['oauth_openid']}' and type=5 order by addtime desc");
    ...
    $awards=pdo_fetchall("select * from ".tablename('hxq_newywym_award')." where uniacid={$uniacid} and cateid in ({$str_cates}) order by id desc",array(),'id');
	        if ($_GPC['type_']==''){
	        $totalsw=pdo_fetchcolumn("select count(id) from ".tablename('hxq_newywym_token')." where uniacid={$uniacid} and isgrant=1 and actid={$act['id']} and oauth_openid='{$_SESSION['oauth_openid']}' and type=4 and veriftime=0 order by addtime desc");
	        $myawardeds=pdo_fetchall("select * from ".tablename('hxq_newywym_token')." where uniacid={$uniacid} and isgrant=1 and actid={$act['id']} and oauth_openid='{$_SESSION['oauth_openid']}' and type=4 and veriftime=0 order by addtime desc");
	        }else{
	        $totalsw=pdo_fetchcolumn("select count(id) from ".tablename('hxq_newywym_token')." where uniacid={$uniacid} and isgrant=1 and actid={$act['id']} and oauth_openid='{$_SESSION['oauth_openid']}' and type=4 and veriftime>0 order by addtime desc");
	        $myawardeds=pdo_fetchall("select * from ".tablename('hxq_newywym_token')." where uniacid={$uniacid} and isgrant=1 and actid={$act['id']} and oauth_openid='{$_SESSION['oauth_openid']}' and type=4 and veriftime>0 order by addtime desc");
	        }	    
	        ...
	        if ($_GPC['type_']==''){
	            ...
	            $mycoupons=pdo_fetchall("select * from ".tablename('hxq_newywym_coupongrant')." where uniacid={$uniacid} and actid={$act['id']} and  bizid={$act['bizid']} and bizid!=0 and oauth_openid='{$_SESSION['oauth_openid']}' and veriftime=0  order by addtime desc",array(),'addtime');
	            ...
	            $coupons=pdo_fetchall('select * from '.tablename('hxq_newywym_coupon')." where uniacid=$uniacid and bizid={$act['bizid']}",array(),'id');
	            $mycoupons=pdo_fetchall("select * from ".tablename('hxq_newywym_coupongrant')." where uniacid={$uniacid} and actid={$act['id']} and  bizid={$act['bizid']} and bizid!=0 and oauth_openid='{$_SESSION['oauth_openid']}'  order by addtime desc",array(),'addtime');
	            foreach($mycoupons as $key=>$row){
	                if($row['veriftime']==0){
	                    if (($coupons[$row['couponid']]['endtime']>TIMESTAMP)){
	                        unset($mycoupons[$key]);
