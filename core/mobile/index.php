...
class index extends page{
    ...
    //echo $_M['slat'];
    $msg=$this->checktoken($this->globalcfg['slat'],$tk,$vtk);
    ...
    if ((!$fan['nickname']||!$fan['avatar'])&&$act['getfaninfo']){
        pdo_update('hxq_newywym_fan',array('nickname'=>$_W['fans']['nickname'],'avatar'=>$_W['fans']['tag']['avatar']),array('uniacid'=>$uniacid,'oauth_openid'=>$_SESSION['oauth_openid']));
    }
    ...
    if (empty($scandata)){
        ...
        if ($award['total']<=$awarded){
            $l_rst=array('type'=>100,'title'=>'奖品已经被抽完','thumb'=>'images/hxq_newywym/unawarded.png','id'=>0);
            $scandata=array(
                'awardid' =>100,
                'type' =>$l_rst['type'],
                'thumb'=>$l_rst['thumb'],
