...
class cate extends page{
      public function main($op='display'){
	      ...
	      if ($op=='export'){
	          ...
	          include $this->template('cate/exportqrcode');
	      }
	      if ($op=='getact'){
	      ...
	      if ($op=='edit'){
	          ...
	          //echo 4;
	          //var_dump($mintk);
	          include $this->template('cate/addcate');
	          exit;
	      }
	      if ($op=='display'){
              ...
              if ($_GPC['actid']){
                  ...
                  $where=" and pid=".$pid;
                  $pcate=pdo_fetch("select * from ".tablename('hxq_newywym_cate')." where uniacid=$uniacid and id={$pid} order by id desc");
