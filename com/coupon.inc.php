...
global $_W,$_GPC,$_M;
    ...
    if ($op=='disable'){
        ...
        if (!empty($id)){
	        ...
	        $start=($page-1)*30;
	        $pagesize=30;
	        $couponverifs=pdo_fetchall("select * from ".tablename('hxq_newywym_coupongrant')." where uniacid={$uniacid} and bizid={$biz['id']}   $where order by id desc limit $start,$pagesize");
	        $pager = pagination($total, $page, 30);
