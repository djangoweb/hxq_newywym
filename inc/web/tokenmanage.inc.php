<?php 
	global $_W,$_GPC,$_M;
	$uniacid=$_W['uniacid'];
	$op=in_array($_GPC['op'],array('display','add','delete','qrcode','download','reset','exportused'))?$_GPC['op']:'display';
	...
	if ($op=='qrcode'){
	    ...
	    $vtk=$_GPC['vtk'];
	    $cateid=$_GPC['cateid'];
	    $url=$_W['siteroot']."addons/".$this->modulename."/scan.php?$tk-$vtk";
    ...
    if (empty($cate)){
	    ...
	    if ($op_=='zip'){
        	...
        	for($i=$cate['mintk']+$start;($i<$cate['mintk']+$start+$pagesize&&$i<=$cate['maxtk']);$i++){
	            $vtk=substr(md5($i.$_M['slat']),0,8);
	            $tokenurl=$_W['siteroot']."addons/".$this->modulename."/scan.php?$i-$vtk";
	            //echo $tokenurl;
	...
	for($i=$cate['mintk']+$start;($i<$cate['mintk']+$start+$pagesize&&$i<=$cate['maxtk']);$i++){
	    ...
	    $tokens[]=array('id'=>$i,
	        'url'=>$_W['siteroot']."addons/".$this->modulename."/scan.php?{$i}-{$vtk}",
