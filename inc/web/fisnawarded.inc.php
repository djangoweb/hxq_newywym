<?php 
	    ...
	    if ($act){
	        ...
	        $awards=pdo_fetchall("select * from ".tablename('hxq_newywym_fisnaward')." where actid={$act['id']} and disable=0 order by id desc",array(),'id');
	        if ($_GPC['aid']){
	            $award=pdo_fetch("select * from ".tablename('hxq_newywym_fisnaward')." where id={$_GPC['aid']} and disable=0 order by id desc");
	    ...
	    if ($op=='export'){
	        ...
	        $awards=pdo_fetchall("select * from ".tablename('hxq_newywym_fisnaward')." where uniacid=$uniacid ",array(),'id');
	        $sql="select * from ".tablename('hxq_newywym_fission')." where  uniacid={$uniacid} $where and (type!=5 and type!=100) order by id asc";
	    ...
	    $total=pdo_fetchcolumn("select count(id) from ".tablename('hxq_newywym_fission')." where uniacid={$uniacid} and isgrant=1 $where and type!=5");
	    ...
	    $awardeds=pdo_fetchall("select * from ".tablename('hxq_newywym_fission')." where uniacid={$uniacid} and isgrant=1  {$where} and type!=5 order by addtime desc {$limit}");
	    unset($row);
	    include $this->template('fisnawarded');
