...
if ($_W['fans']['uid']){
    ...
    if ($_W['account']['level']==4&&$_W['oauth_account']['acid']==$_W['account']['acid']){
        ...
        $totalcoupon=pdo_fetchcolumn('select count(*) from '.tablename('hxq_newywym_coupongrant')." a left join ".tablename('hxq_newywym_coupon')." as b on a.couponid=b.id where a.uniacid={$uniacid}  and a.actid={$act['id']} and a.veriftime=0 and b.endtime>unix_timestamp() and a.oauth_openid='{$_SESSION['oauth_openid']}'");
