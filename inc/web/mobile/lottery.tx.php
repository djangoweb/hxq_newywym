<?php
    if($tokendata['type']==1){
        $fanrps=pdo_fetchall('select redpack,id from '.tablename('hxq_newywym_token')." where uniacid=$uniacid and actid={$_M['act']['id']}  and type=1  and isgrant=0 and oauth_openid='{$_SESSION['oauth_openid']}' ",array(),'id');
        $fanrps=array_map('array_shift',$fanrps);
        $totalrp=array_sum($fanrps);
        $rpjkeys=array_keys($fanrps);
        $rpmessage='累积  '.number_format($totalrp,2).'元';
        if ($act['getcashleast']>1){
            $hmessage='累积满 '.$act['getcashleast'].'可在个人中心提现';
        }else{
            $hmessage='累积满 1元可在个人中心提现';
        }
        if ($totalrp<1){
        ...
        if ($act['gcredit1']){
            ...
            $rec['isgrant']=1;
            pdo_insert('hxq_newywym_give',$rec);
            $cmsg=array('errno'=>0,'message'=>'赠送积分失败，请注册会员后在个人中心领取');
        } 
        if ($act['page']['template']=='activity_v1'){
                include $this->template('lottery');
            }else{
        ...
        if (is_error($rp_rst)){
            ...
            pdo_update('hxq_newywym_token',array('isgrant'=>1,'send_listid'=>$rp_rst['send_listid']),array('id'=>$tokenid));
